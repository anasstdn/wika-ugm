    ...
    public function store(Request $request)
    {
        ...
        try {
            if ($request->input('check', null) !== null && count($request->input('check', null)) > 0) {
                ...
                $data_riwayat_survei = array(
                    'survei_id' => $survei_id,
                    'action_id' => getConfigValues('ACTION_CREATE')[0],
                    'user_input' => \Auth::user()->id,
                    'datetime_log' => current_datetime(),
                    'description' => array(
                        'action' => 'User ' . getProfileByUserId(\Auth::user()->id)->nama . ' telah membuat Survei Barang'
                    ),
                );

                $insert_riwayat_survei = RiwayatSurvei::create($data_riwayat_survei);
